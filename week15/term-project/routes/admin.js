...
const specialOfferValidation = [
  body("specialOffer").trim().isLength({ min: 1, max: 200 }).escape(),
  body("specialOfferDetails").trim().isLength({ min: 1, max: 1000 }).escape()
];

const deleteMessagesValidation = [
  body("messageIds").optional().customSanitizer(value => Array.isArray(value) ? value : [value])
];
...
router.post("/special-offer", isAuthenticated, specialOfferValidation, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).send("Invalid special offer input");
  }
...
router.post("/contact-messages/delete-selected", isAuthenticated, deleteMessagesValidation, async (req, res) => {
  try {
    const property = await Property.findOne({ name: "Lava Birds B&B" });
