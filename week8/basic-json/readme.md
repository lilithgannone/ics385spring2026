...
I began this assignment by reviewing the existing code listed in the assignment details PDF. I then reviewed the full PDF and uploaded the PDF document to Codex as a .txt file to set up the starter code. Next, I assessed all the changes that needed to be made. I started by updating the json file, reviewing the structure, and adding in the course data using the UH Maui course catalog. I prompted Codex to make these changes using a lengthy prompt describing what needed to be done. I then began propting Codex on VS Code to make the needed changes detailed in the "Step by Step Implementation" guide. Throughout this process, I tested the program on a desktop. I then tested the program on mobile using a Chrome extension. I verified the testing requirements via manual tests and AI-run tests. I then did some fixes and asked that comments be made to the code explaining all functionalities. I reviewed the code, made some additional changes, and then prompted for a final review to ensure all needed requirements had been met. I made a couple of final fixes and then completed an AI code review. I ran the program to ensure CSS met the needed requirements and that the program was still functional. All requirements were met, so I reviewed the code and added some manual comments labeled "comment" using what I learned from W3Schools. I then uploaded the basic-json folder to GitHub. 
